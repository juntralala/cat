...
<template>
  ...
  <v-container>
    ...
    <v-row>
      <v-col>
        <v-btn variant="tonal" color="blue-darken-2">
          ...
          <v-dialog v-slot="{ isActive }" v-model="showUserAddDialog" activator="parent" max-width="800">
            <v-card>
              ...
              <v-card-text>
                <v-form ref="userAddFormRef">
                  ...
                  <v-select v-model="userAddForm.role" :items="roles" item-title="name" density="comfortable"
                    :rules="userAddFormRule.role" item-value="name" label="Role" />
                </v-form>
              </v-card-text>
              <v-card-actions>
                <v-btn @click="isActive.value = false">Cancel</v-btn>
                <v-btn color="blue-darken-4" :disabled="disableUsetAddSubmit" @click="submitUserAddForm">
    ...
    <!-- Edit User Dialog -->
    <v-dialog v-model="showUserEditDialog" max-width="800">
      <v-card>
        ...
        <v-card-text>
          <v-form ref="userEditFormRef">
            ...
            <v-text-field v-model="userEditForm.password" label="Password (kosongkan jika tidak diubah)"
              density="comfortable" :rules="userEditFormRule.password" :type="showEditPassword ? 'text' : 'password'"
              :append-inner-icon="showEditPassword ? 'mdi-eye' : 'mdi-eye-off'"
              @click:append-inner="toggleShowEditPassword" />
            <v-select v-model="userEditForm.role" :items="roles" item-title="name" density="comfortable"
              :rules="userEditFormRule.role" item-value="name" label="Role" />
          </v-form>
        </v-card-text>
        <v-card-actions>
          <v-btn @click="showUserEditDialog = false">Cancel</v-btn>
          <v-btn color="blue-darken-4" :disabled="disableUserEditSubmit" @click="submitUserEditForm">
    ...
    <v-row class="hidden! md:block!">
      <v-col>
        <v-data-table-server :headers="[
          ...
          { title: 'Role', key: 'role' },
          { title: 'More', key: 'more' }
        ]" :items="users" :items-length="totalItems" :loading="loading" class="hidden! md:block!"
            ...
            <v-btn variant="text" icon>
              ...
              <v-menu activator="parent">
                <v-list density="compact">
                  <v-list-item value="edit" :disabled="isCurrentUser(item.more)" @click="openEditDialog(item)">
                    <v-icon icon="mdi-pencil" class="mr-2" />
                    Sunting
                  </v-list-item>
                  <v-list-item value="delete" :disabled="isCurrentUser(item.more)">
                    ...
                    <v-dialog v-slot="{ isActive }" activator="parent" max-width="400">
                      <v-card>
                        ...
                        <v-card-text>
                          <div>Apakah yakin untuk menghapus pengguna dengan nama <span class="text-blue-600">{{
                            item.name }}</span></div>
                        </v-card-text>
                        <v-card-actions>
    ...
    <v-row class="md:hidden!">
      <v-col>
            ...
            <v-card>
              <v-card-actions class="flex justify-end bg-blue-darken-2">
                <v-btn variant="text" icon>
                  ...
                  <v-menu activator="parent">
                    <v-list density="compact">
                      <v-list-item value="edit" :disabled="isCurrentUser(user.more)" @click="openEditDialog(user)">
                        <v-icon icon="mdi-pencil" class="mr-2" />
                        Sunting
                      </v-list-item>
                      <v-list-item value="delete" :disabled="isCurrentUser(user.more)">
                        <v-icon icon="mdi-delete" class="mr-2" />
                        Hapus
                        <v-dialog v-slot="{ isActive }" activator="parent" max-width="400">
                          <v-card>
        ...
        <v-row v-if="!loading && users.length > 0">
          <v-col>
            <v-pagination v-model="currentPage" :length="Math.ceil(totalItems / itemsPerPage)" total-visible="5"
              @update:model-value="loadItems({ page: currentPage, itemsPerPage })" />
          </v-col>
        </v-row>
      </v-col>
    </v-row>
