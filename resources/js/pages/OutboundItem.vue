...
onBeforeMount(() => {
  props.recipients.map((item) => (item.name_nickname = item.nickname ? `${item.name} (${item.nickname})` : item.name));
});
onBeforeUpdate(() => {
  props.recipients.map((item) => (item.name_nickname = item.nickname ? `${item.name} (${item.nickname})` : item.name));
});
    ...
    <v-row>
      <v-col>
        <form @submit.prevent="submitTransaction">
          ...
          <v-row class="pt-4">
            ...
            <v-col cols="12" md="4">
              <v-text-field
                ...
                :error-messages="form.errors.division"
                persistent-hint
              />
            </v-col>
            <v-col cols="12" md="4">
          ...
          <v-row>
            <v-col cols="12">
              <v-textarea v-model="form.notes" density="comfortable" label="Catatan (opsional)" rows="2" :error-messages="form.errors.notes" />
            </v-col>
          </v-row>
          ...
          <v-row v-for="(item, index) in form.transaction_details" :key="index" class="items-start!">
            <v-col class="py-0!">
              <v-autocomplete
                ...
                item-value="id"
                label="Nama Barang"
                :error-messages="form.errors[`transaction_details.${index}.item_id`]"
              ></v-autocomplete>
            </v-col>
            ...
            <v-col cols="auto" class="mt-1 py-0!">
              <v-btn @click="deleteItem(index)" variant="tonal" color="blue" icon size="small" :disabled="form.transaction_details.length === 1">
          ...
          <v-row>
            <v-col class="flex justify-end gap-4">
              ...
              <v-btn type="submit" variant="flat" color="blue-accent-3" :loading="form.processing" :disabled="form.processing">
