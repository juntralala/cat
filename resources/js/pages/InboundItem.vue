...
<template>
    <v-container fluid>
        ...
        <v-dialog v-model="successDialog" max-width="400" persistent>
            <v-card>
                <v-card-text class="text-center pa-8">
                    ...
                    <p class="text-body-1">Transaksi barang masuk berhasil disimpan.</p>
                </v-card-text>
        ...
        <v-row>
            <v-col>
                <h1 class="text-4xl font-medium">Barang <span class="text-blue-accent-3">Masuk</span></h1>
            </v-col>
        </v-row>
        <v-row>
            <v-col>
                <form @submit.prevent="submitTransaction">
                        ...
                        <v-col cols="12" md="6">
                            <v-menu v-model="menuDate" :close-on-content-click="false" transition="scale-transition"
                                ...
                                <v-date-picker v-model="form.transaction_date" @update:model-value="menuDate = false"
                                    locale="id" header="Pilih Tanggal" hide-header :max="new Date()" />
                            </v-menu>
                        </v-col>
                    </v-row>
                    ...
                    <v-row v-for="(item, index) in form.transaction_details" :key="index" class="items-start!">
                        ...
                        <v-col cols="auto" class="mt-1 py-0!">
                            <v-btn @click="deleteItem(index)" variant="tonal" color="red" icon size="small"
